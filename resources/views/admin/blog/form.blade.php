...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 space-y-6">
        ...
        <div>
            ...
            <textarea id="content" name="content" rows="20" required
                class="w-full rounded-xl border border-gray-300 px-4 py-3 focus:border-indigo-500 focus:ring-indigo-500">{{ old('content', $post->content ?? '') }}</textarea>
        </div>
    </div>
    ...
    <div class="space-y-6">
        <div class="rounded-2xl border border-gray-200 p-4 bg-gray-50">
            ...
            <select id="status" name="status" class="w-full rounded-xl border border-gray-300 px-3 py-2 focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <label for="published_at" class="block text-sm font-semibold text-gray-700 mt-4 mb-1">Publish Date</label>
            <input id="published_at" name="published_at" type="datetime-local"
                value="{{ old('published_at', isset($post->published_at) && $post->published_at ? $post->published_at->format('Y-m-d\\TH:i') : '') }}"
                class="w-full rounded-xl border border-gray-300 px-3 py-2 focus:border-indigo-500 focus:ring-indigo-500">
        </div>
        ...
        <div class="rounded-2xl border border-gray-200 p-4 bg-gray-50">
            ...
            @if(!empty($post?->featured_image_path))
                <div class="mt-4">
                    <img src="{{ asset('storage/' . $post->featured_image_path) }}" alt="Current featured image" class="w-full rounded-lg border border-gray-200">
        ...
        <div class="rounded-2xl border border-gray-200 p-4 bg-gray-50">
            ...
            <label for="meta_description" class="block text-sm font-semibold text-gray-700 mt-4 mb-1">Meta Description</label>
            ...
            <label for="excerpt" class="block text-sm font-semibold text-gray-700 mt-4 mb-1">Excerpt</label>
